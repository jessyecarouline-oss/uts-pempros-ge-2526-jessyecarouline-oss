...
int find_item(char *id) {
    for (int i = 0; i < item_count; i+++) {
        if(strcmp(items[1].id, id) == 0) return i;
...
int main() {
    ...
    while(1) {
        ...
        if (strcmp (cmd , "recieve") == 0) {
            ...
            Item[item_count].qty = qty;
            Item[item_count].price = price;

            item_count++;
        }

        else if (strmp(cmp, "report") == 0) {
            for (int i = 0; i < item_count; i++) {
                printf("%s|%s|%d|%.1f\n",
                       items[i].id,
                       items[i].name,
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(cmd, "receive") == 0) {
            ...
            if (items[idx].quantity < qty) continue;
